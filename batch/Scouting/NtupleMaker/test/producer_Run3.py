...
opts.register('syst',    False,       mytype = vpbool) # Adding option for systematics 
opts.parseArguments()
...
if '2022' in opts.era or (opts.data and '2023B' in opts.era) or '2023C-triggerV10' in opts.era:
    ...
    # Add orthogonal triggers for systematics
    if opts.syst:
...
# Note: Avoid skimming if syst
if (opts.data) and not (opts.syst): process.skimpath = cms.Path(process.countmu+process.countvtx+process.gtStage2Digis+process.triggerMaker+process.offlineBeamSpot+process.beamSpotMaker+process.MeasurementTrackerEvent+process.hitMaker)
