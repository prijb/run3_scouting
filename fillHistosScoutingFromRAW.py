...
parser.add_argument("--inDir", default="/ceph/cms/store/user/evourlio/Run3ScoutingSamples/Data/", help="Choose input directory. Default: '/ceph/cms/store/user/evourlio/Run3ScoutingSamples/Data/'")
...
parser.add_argument("--partialUnblindingFraction", default="0.05", help="Fraction of available data to be processed")
...
if args.inSample=="DataB":
    ...
    sampledir = "2022G_2"
### Signal
elif args.inSample=="Signal_ScenA_v0p30":
    sampledir = "/DarkShower_ScenarioA_default_Run3Summer22GS_v0p30_AODSIM_v0p30/crab_skim__2022_DarkShower_ScenarioA_default_Run3Summer22GS_v0p30_AODSIM_v0p30/230703_184107/0000/"
elif args.inSample=="Signal_ScenA_v1p3":
    sampledir = "/DarkShower_ScenarioA_default_Run3Summer22GS_v1p3_AODSIM_v1p3/crab_skim__2022_DarkShower_ScenarioA_default_Run3Summer22GS_v1p3_AODSIM_v1p3/230703_184125/0000/"
elif args.inSample=="Signal_ScenA_v1p4":
    sampledir = "/DarkShower_ScenarioA_default_Run3Summer22GS_v1p4_AODSIM_v1p4/crab_skim__2022_DarkShower_ScenarioA_default_Run3Summer22GS_v1p4_AODSIM_v1p4/230703_184144/0000/"
elif args.inSample=="Signal_ScenA_v1p5":
    sampledir = "/DarkShower_ScenarioA_default_Run3Summer22GS_v1p5_AODSIM_v1p5/crab_skim__2022_DarkShower_ScenarioA_default_Run3Summer22GS_v1p5_AODSIM_v1p5/230703_184201/0000/"
elif args.inSample=="Signal_ScenB_v0p32":
    sampledir = "/DarkShower_ScenarioB_default_Run3Summer22GS_v0p32_AODSIM_v0p32/crab_skim__2022_DarkShower_ScenarioB_default_Run3Summer22GS_v0p32_AODSIM_v0p32/230703_184218/0000/"
elif args.inSample=="Signal_ScenC_v0p34":
    sampledir = "/DarkShower_ScenarioC_default_Run3Summer22GS_v0p34_AODSIM_v0p34/crab_skim__2022_DarkShower_ScenarioC_default_Run3Summer22GS_v0p34_AODSIM_v0p34/230703_184235/0000/"
...
if args.inFile!="*" and args.inSample!="*":
    thisfile="output_%s.root"%(args.inFile)
    if os.path.isfile("/ceph/cms%s%s/%s"%(indir,sampledir,thisfile)):
        files.append("%s%s%s/%s"%(prependtodir,indir,sampledir,thisfile))
elif args.inSample!="*":
    for f in os.listdir("/ceph/cms%s%s"%(indir,sampledir)):
        if (".root" in f) and os.path.isfile("/ceph/cms%s%s/%s"%(indir,sampledir,f)):
            files.append("%s%s%s/%s"%(prependtodir,indir,sampledir,f))
else:
    for f in os.listdir("/ceph/cms%s"%indir):
        if (".root" in f) and os.path.isfile("/ceph/cms%s/%s"%(indir,f)):
...
# L1 results
l1rhandle = Handle('std::vector<std::bool>')
l1rlabel  = ("triggerMaker","l1result")
...
for en,e in enumerate(events):
    ...
    e.getByLabel(l1rlabel,l1rhandle)
    l1sResults = l1rhandle.product()
    passl1t = False
    l1seeds = []
    ...
    for l1sn,l1s in enumerate(l1sStrings):
        if l1sStrings[l1sn] in l1seeds and l1sPrescales[l1sn]==1 and l1sResults[l1sn]:
            passl1t=True
            break
    if not passl1t:
        continue
